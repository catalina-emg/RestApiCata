...
class AuthController {
    ...
    public function register() {
        ...
        Logger::info('POST /auth/register payload: ' . json_encode($input));
        ...
        if (!preg_match('/^[\p{L}\s]+$/u', $nombre)) {
            http_response_code(400);
            Logger::warn("Registro fallido - Nombre inválido: $nombre");
            echo json_encode([
                'success' => false,
    ...
    public function login() {
        $input = json_decode(file_get_contents("php://input"), true);
        Logger::info('POST /auth/login payload: ' . json_encode(['email' => $input['email'] ?? '']));
